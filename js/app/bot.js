...
const bot = {
  ...
  randomMove(currentBoard) {
    const indexes = Array.from(currentBoard).reduce(
    ...
    return selectOne(indexes);
  },

  async findMove(currentBoard) {
    await delayExecution(750, currentBoard);
    ...
    if (winningMoves.length > 0) {
      return currentBoard[bot.chooseMove(winningMoves)];
    }
